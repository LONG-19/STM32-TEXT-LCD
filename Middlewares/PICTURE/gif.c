...
static int gif_getnextcode(FIL *filename, gif89a *gif)
{
    ...
    {
        Result = (long)gif->lzw->aBuffer[i] | ((long)gif->lzw->aBuffer[i + 1] << 8) | ((long)gif->lzw->aBuffer[i + 2] << 16);
    }
    ...
    if (res == 0)                               /* OK */
    {
        ...
        if (res == 0)                           /* ���ļ�ok */
        {
            ...
            while (g_gif_decoding && res == 0)                                  /* ����ѭ�� */
            {
                ...
                if (mygif89a->delay)
                {
                    dtime = 2;                                    /* ��ȡ��ʱʱ��(��λ 10ms) mygif89a->delay*/
                }
                else 
                {
                    dtime = 2;                                                 /* Ĭ����ʱ(100ms) */		//33
                }
