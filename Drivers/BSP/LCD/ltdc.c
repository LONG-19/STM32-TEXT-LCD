 ...
 * @attention
 *
    uint32_t ltdc_lcd_framebuf[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR)));                        /* RGB888/ARGB8888 frame buffer 0 */
    uint32_t ltdc_lcd_framebuf1[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR + 1280 * 800 * 4)));      /* RGB888/ARGB8888 frame buffer 1 */
    uint16_t ltdc_lcd_framebuf[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR)));                        /* RGB565 frame buffer 0 */
    uint16_t ltdc_lcd_framebuf1[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR + 1280 * 800 * 2)));      /* RGB565 frame buffer 1 */
    uint32_t ltdc_lcd_framebuf[1280][800] __attribute__((section(".bss.ARM.__at_0XC0000000")));                       /* RGB888/ARGB8888 frame buffer 0 */
    uint32_t ltdc_lcd_framebuf1[1280][800] __attribute__((section(".bss.ARM.__at_0XC0000000 + 0x3E8000")));           /* RGB888/ARGB8888 frame buffer 1 */
    uint16_t ltdc_lcd_framebuf[1280][800] __attribute__((section(".bss.ARM.__at_0XC0000000")));                       /* RGB565 frame buffer 0 */
    uint16_t ltdc_lcd_framebuf1[1280][800] __attribute__((section(".bss.ARM.__at_0XC0000000 + 0x177000")));          /* RGB565 frame buffer 1 */
static uint8_t g_ltdc_front_buf = 0;       /* front buffer index */
static uint8_t g_ltdc_back_buf  = 1;       /* back buffer index */
/**************************************************************************/
/**
 * @brief       Set drawing buffer index
 * @param       backbuf    : back buffer index
 */
void ltdc_set_draw_buffer(uint8_t backbuf)
{
    if (backbuf < 2)
    {
        lcdltdc.activelayer = backbuf;
        g_ltdc_back_buf = backbuf;
    }
}

/**
 * @brief       Get back buffer index
 * @retval      Back buffer index
 */
uint8_t ltdc_get_back_buffer(void)
{
    return g_ltdc_back_buf;
}

/**
 * @brief       Flip LTDC front/back buffers
 * @note        Updates LTDC address to the back buffer then swaps indices.
 */
void ltdc_flip_buffers(void)
{
    uint8_t new_front = g_ltdc_back_buf;

    HAL_LTDC_SetAddress(&g_ltdc_handle, (uint32_t)g_ltdc_framebuf[new_front], 0);
    __HAL_LTDC_RELOAD_CONFIG(&g_ltdc_handle);

    g_ltdc_back_buf = g_ltdc_front_buf;
    g_ltdc_front_buf = new_front;

    ltdc_set_draw_buffer(g_ltdc_back_buf);
}
...
void ltdc_init(void)
{
    ...
    ltdc_set_draw_buffer(g_ltdc_back_buf); /* switch drawing to back buffer */
    ltdc_clear(0XFFFFFFFF);                /* clear back buffer */

    /* Default to front display with background rendering */
