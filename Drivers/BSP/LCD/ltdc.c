 ...
 * @attention
 *
    uint32_t ltdc_lcd_framebuf[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR1)));  /* ֱʱ,LTDC֡С */
    uint32_t ltdc_lcd_framebuf1[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR2))); /* ֱʱ,LTDC֡С */
    uint16_t ltdc_lcd_framebuf[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR1)));  /* ֱʱ,LTDC֡С */
    uint16_t ltdc_lcd_framebuf1[1280][800] __attribute__((at(LTDC_FRAME_BUF_ADDR2))); /* ֱʱ,LTDC֡С */
    uint32_t ltdc_lcd_framebuf1[1280][800] __attribute__((section(".bss.ARM.__at_0XC03E8000"))); /* ֱʱ,LTDC֡С */
    uint16_t ltdc_lcd_framebuf1[1280][800] __attribute__((section(".bss.ARM.__at_0XC01F4000"))); /* ֱʱ,LTDC֡С */
...
/**
 * @brief       ָǰƵ֡
 * @param       bufindex    : Ҫʹõ֡(0/1)
 * @retval
 */
void ltdc_set_draw_buffer(uint8_t bufindex)
{
    if (bufindex < 2)
    {
        lcdltdc.drawbuf = bufindex;
    }
}
 ...
 * @retval
 */
void ltdc_switch_frame(void)
{
    if (lcdltdc.frontbuf != lcdltdc.drawbuf)
    {
        uint8_t oldfront = lcdltdc.frontbuf;
        HAL_LTDC_SetAddress(&g_ltdc_handle, (uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf], lcdltdc.activelayer);
        HAL_LTDC_Reload(&g_ltdc_handle, LTDC_RELOAD_VERTICAL_BLANKING);
        lcdltdc.frontbuf = lcdltdc.drawbuf;
        lcdltdc.drawbuf = oldfront;
    }
}

        *(uint32_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * y + x)) = color;
        *(uint32_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * (lcdltdc.pheight - x - 1) + y)) = color; 
        *(uint16_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * y + x)) = color;
        *(uint16_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * (lcdltdc.pheight - x - 1) + y)) = color; 
        return *(uint32_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * y + x));
        return *(uint32_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * (lcdltdc.pheight - x - 1) + y)); 
        return *(uint16_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * y + x));
        return *(uint16_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * (lcdltdc.pheight - x - 1) + y)); 

/**
    addr = ((uint32_t)g_ltdc_framebuf[lcdltdc.drawbuf] + lcdltdc.pixsize * (lcdltdc.pwidth * psy + psx));
 * @param       dir          : 0,竖屏；1,横屏
 * @retval      无
 */
...
#endif
    lcdltdc.frontbuf = 0;
    lcdltdc.drawbuf = lcdltdc.frontbuf ^ 1;

    ltdc_layer_parameter_config(0, (uint32_t)g_ltdc_framebuf[lcdltdc.frontbuf], LTDC_PIXFORMAT, 255, 0, 6, 7, 0X000000);   /*  */
    ltdc_set_draw_buffer(lcdltdc.frontbuf);
    ltdc_clear(0XFFFFFFFF);                /* ǰ֡ */
    ltdc_set_draw_buffer(lcdltdc.drawbuf);
    ltdc_clear(0XFFFFFFFF);                /* պ̨֡ */
