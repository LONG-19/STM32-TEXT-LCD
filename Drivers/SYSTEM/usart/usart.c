...
/* 让串口中断可以改 main.c 里的命令变量 */
extern volatile uint8_t g_gif_cmd;
extern volatile uint8_t g_gif_cmd_flag;
...
/* AA 55 00 XX FB 协议解析状态机 */
typedef enum
{
    UART_STATE_WAIT_AA = 0,
    UART_STATE_WAIT_55,
...
static uart_rx_state_t s_uart_state = UART_STATE_WAIT_AA;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        uint8_t b = g_rx_buffer[0];   /* 本次收到的 1 个字节 */

        switch (s_uart_state)
        {
        case UART_STATE_WAIT_AA:
            if (b == 0xAA)
            {
                s_uart_state = UART_STATE_WAIT_55;
            }
            break;

        case UART_STATE_WAIT_55:
            if (b == 0x55)
            {
                s_uart_state = UART_STATE_WAIT_TYPE;
            }
            else if (b == 0xAA)
            {
                /* 仍然当作帧头第1字节 */
            ...
            {
                s_uart_state = UART_STATE_WAIT_AA;
            }
            break;
        ...
        case UART_STATE_WAIT_TYPE:
            if (b == 0x00)
            {
                /* 目前只用 00 这一种类型，表示“切换表情” */
                s_uart_state = UART_STATE_WAIT_DATA;
            }
            else
            {
                /* 其它类型先忽略掉 */
        ...
        case UART_STATE_WAIT_DATA:
            /* 保存数据字节，等尾字节 */
            s_uart_data  = b;
            s_uart_state = UART_STATE_WAIT_FB;
            break;

        case UART_STATE_WAIT_FB:
            if (b == 0xFB)
            {
                /* 收到一帧完整数据：AA 55 00 s_uart_data FB */
                g_gif_cmd      = s_uart_data;
                g_gif_cmd_flag = 1;
            }
            /* 不管是否成功，都重新等待下一帧 */
            s_uart_state = UART_STATE_WAIT_AA;
